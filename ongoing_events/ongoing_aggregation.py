...
Pure by design, same discipline as the historical aggregate(): hand these
functions fake contact data and they return the three company property values,
with no HubSpotClient involved. See test_ongoing_aggregation.py.

Input is the two contact properties Ops maintains by hand:
...
NOTE ON WHAT IS NOT HERE. There is no List ID, no List Role, and no
COUNT_HIGH_ENGAGEMENT_AS_ATTENDANCE. Those are backfill-only concepts that
existed because List membership was the only data source for the backfill
period. Ops now maintains high engagement directly on the contact, so an event
is just an event and the only thing that varies is its Channel/General tier.
If a change here starts needing List semantics, that is a sign of backfill
logic leaking into the ongoing path.
"""
...
# Registry tiers are "Channel" / "General"; the HubSpot property options are the
...
class UnmatchedEventError(OngoingAggregationError):
    ...
    def __init__(self, unmatched: list[UnmatchedEvent]) -> None:
        ...
        lines.append(
            "Add a row for each event to input/marketingEventsRegistry.csv "
            "(with its Channel/General tier), or fix the typo on the contact, "
...
def compute_company_properties(
    ...
    tier_lookup         : canonical_event -> "Channel" | "General", from
                          shared.aggregation.event_tier_lookup().
    ...
    registry gap rather than making Ops fix them one at a time.
    """
